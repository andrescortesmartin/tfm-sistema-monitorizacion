...
{
  //ALIMENTACION
  hal_gpio_init_out(LR1110_POWER, HAL_GPIO_RESET); //LR1110 OFF
  hal_gpio_init_out(GPS_POWER, HAL_GPIO_RESET); //GPS OFF 
  hal_gpio_init_out(LR1110_GNSS_ANT_PIN, HAL_GPIO_RESET); // LNA apagado
  hal_gpio_init_out(SENSOR_POWER, HAL_GPIO_RESET); //SENSORES OFF
  ...
  //LEDS RGB APAGADOS 
  hal_gpio_init_out(LED_R, HAL_GPIO_SET);

  //PULSADOR 
  hal_gpio_init_in(SWITCH, HAL_GPIO_PULL_MODE_NONE, HAL_GPIO_IRQ_MODE_OFF, NULL);

  //BATERIA
  hal_gpio_init_in(VBAT_VALUE, HAL_GPIO_PULL_MODE_NONE, HAL_GPIO_IRQ_MODE_OFF, NULL);//valor del divisor de tension
  hal_gpio_init_in(BATTERY_STATUS, HAL_GPIO_PULL_MODE_UP, HAL_GPIO_IRQ_MODE_OFF, NULL);//status de carga
  hal_gpio_init_in(VBAT_CTRL, HAL_GPIO_PULL_MODE_NONE, HAL_GPIO_IRQ_MODE_OFF, NULL); //control de medida de bateria (NO MEDIR)
  
  //SENSOR LSM6DSOX
  hal_gpio_init_out( LSM6DSOX_SPI_CS_PIN, HAL_GPIO_RESET );
  hal_gpio_init_out( LSM6DSOX_SPI_SCK_PIN, HAL_GPIO_RESET );
  hal_gpio_init_out( LSM6DSOX_SPI_MOSI_SDI_PIN, HAL_GPIO_RESET );
  hal_gpio_init_out( LSM6DSOX_SPI_MISO_SDO_PIN, HAL_GPIO_RESET );
  hal_gpio_init_out( LSM6DSOX_INTERRUPT1, HAL_GPIO_RESET );
  hal_gpio_init_out( LSM6DSOX_INTERRUPT2, HAL_GPIO_RESET );
  //hal_gpio_init_in( LSM6DSOX_INTERRUPT1, HAL_GPIO_PULL_MODE_DOWN,HAL_GPIO_IRQ_MODE_OFF,NULL);
  //hal_gpio_init_in( LSM6DSOX_INTERRUPT2, HAL_GPIO_PULL_MODE_DOWN,HAL_GPIO_IRQ_MODE_OFF,NULL);

  //SENSOR LPS22DF
  //hal_gpio_init_in( LPS22DF_INTERRUPT, HAL_GPIO_PULL_MODE_DOWN,HAL_GPIO_IRQ_MODE_OFF,NULL);
  hal_gpio_init_out( LPS22DF_INTERRUPT, HAL_GPIO_RESET );
