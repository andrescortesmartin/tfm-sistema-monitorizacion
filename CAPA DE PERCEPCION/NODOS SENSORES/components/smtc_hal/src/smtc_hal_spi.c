...
static const nrfx_spim_t spi_lsm6dsox = NRFX_SPIM_INSTANCE(1);

static uint8_t m_tx_buf[256] = { 0 };
static uint8_t m_rx_buf[256] = { 0 };

static bool spi_init = false;
static bool spi_lsm6dsox_init = false;


//--------------------//
//FUNCIONES DEL LR1110//
//--------------------//
...
{
    if( spi_lsm6dsox_init == false )
    {
        spi_lsm6dsox_init = true;
        nrfx_spim_config_t spi_config = NRFX_SPIM_DEFAULT_CONFIG;
        spi_config.frequency = NRF_SPIM_FREQ_125K;
        spi_config.mode = NRF_SPIM_MODE_3;
        spi_config.bit_order = NRF_SPIM_BIT_ORDER_MSB_FIRST;
        spi_config.miso_pin = LSM6DSOX_SPI_MISO_SDO_PIN;
        spi_config.mosi_pin = LSM6DSOX_SPI_MOSI_SDI_PIN;
        spi_config.sck_pin = LSM6DSOX_SPI_SCK_PIN;
        spi_config.use_hw_ss = false;
        spi_config.ss_active_high = false;
        nrfx_spim_init( &spi_lsm6dsox, &spi_config, NULL, NULL );
    }
}

void hal_spi_lsm6dsox_deinit( void )
{
    if( spi_lsm6dsox_init == true )
    {
        spi_lsm6dsox_init = false;
        nrfx_spim_uninit( &spi_lsm6dsox );
    }
}
...
void hal_spi_lsm6dsox_write( const uint8_t* buffer, uint16_t length )
{
    ...
    {
        nrfx_spim_xfer_desc_t xfer_desc = NRFX_SPIM_XFER_TX( buffer, length );
        nrfx_spim_xfer( &spi_lsm6dsox, &xfer_desc, NRFX_SPIM_FLAG_NO_XFER_EVT_HANDLER );
    }
}
