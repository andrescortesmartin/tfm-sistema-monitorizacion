...
fds_record_t m_record = {.file_id = CONFIG_FILE1,
                         .key = CONFIG_REC_KEY1,
                         .data.p_data = &configuracion_app,
                         .data.length_words = (sizeof(logger_config_t) + 3) / 4
                        };
...
void app_fds_init(void) {
  ...
  if (rc == NRF_SUCCESS) {
    // Encontramos datos guardados y los copiamos a RAM
    fds_flash_record_t temp = {0};
    rc = fds_record_open(&desc, &temp);
    if (rc == NRF_SUCCESS) {
      // Recuperamos los datos tras un reinicio
      memcpy(&configuracion_app, temp.p_data, sizeof(logger_config_t));
      PRINTF("Configuracion RECUPERADA de Flash\n");
      fds_record_close(&desc);
    }
  } else {
    // No hay datos. Guardamos los valores por defecto
...
/** @brief Detecta y recicla espacio desperdiciado en flash */
static void waste_detect_recycle(void) {
  ...
  // Si queda poco espacio (menos de 3 veces el tamaño de config), limpiamos basura
  if (stat.largest_contig < ((sizeof(logger_config_t) + 3)/4)*3) {
